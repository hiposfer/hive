...
function loadApp(platform, devHost, onLoadCb) {
    // NOTE: when using source-maps isChrome() returns true, which means we
    // would use 'localhost' but that doesnt work in the phone :(
    //var fileBasePath = serverBaseUrl((isChrome() ? "localhost" : devHost)) + "/" + config.basePath + platform;
    var fileBasePath = serverBaseUrl(devHost) + "/" + config.basePath + platform;
